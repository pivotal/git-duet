...
describe Git::Duet::SoloCommand do
  ...
  it 'sets the soloist name as git config user.name' do
    ...
    cmd.stub(:`).with(/git config --unset-all #{Git::Duet::CONFIG_NAMESPACE}/)
    cmd.should_receive(:`)
  ...
  it 'unsets the committer name' do
    cmd.stub(:`).with(/git config user\.name/)
    cmd.stub(:`).with(/git config user\.email/)
    cmd.stub(:`).with(/git config --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-email/)
    cmd.should_receive(:`)
      .with("git config --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-name")
    cmd.execute!
  end
  ...
  it 'unsets the committer email' do
    cmd.stub(:`).with(/git config user\.name/)
    cmd.stub(:`).with(/git config user\.email/)
    cmd.stub(:`).with(/git config --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-name/)
    cmd.should_receive(:`)
      .with("git config --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-email")
    cmd.execute!
  end
  ...
  context 'when configured to operate on the global config' do
    ...
    it 'sets the soloist name as global git config user.name' do
      ...
      cmd.stub(:`).with(/git config --global --unset-all #{Git::Duet::CONFIG_NAMESPACE}/)
    ...
    it 'unsets the global committer name' do
      ...
      cmd.stub(:`).with(/git config --global user\.email/)
      cmd.stub(:`)
        .with(/git config --global --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-email/)
      cmd.should_receive(:`)
        .with("git config --global --unset-all #{Git::Duet::CONFIG_NAMESPACE}.git-committer-name")
      cmd.execute!
    end
