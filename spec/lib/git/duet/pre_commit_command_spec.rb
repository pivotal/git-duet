...
describe Git::Duet::PreCommitCommand do
  ...
  it 'does nothing if the env cache is not stale' do
    cmd.stub(:exec_check).with(/git config duet\.env\.git/)
    cmd.stub(:exec_check).with('git config duet.env.mtime').and_return(Time.now.to_i)
  ...
  it 'explodes if the env cache does not exist' do
    cmd.stub(:exec_check).with(/git config duet\.env\.git/)
    cmd.stub(:exec_check).with('git config duet.env.mtime').and_raise(StandardError)
